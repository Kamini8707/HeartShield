...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://heartshield_user:hs1234@localhost/heartshield'
...
def extract_key_values(text):
    ...
    for key, pattern in patterns.items():
        ...
        if matches:
            if key == "systolic":
                val = matches.group(1)
                if is_valid_medical_range('ap_hi', val):
                    results['ap_hi'] = val
            elif key == "diastolic":
                val = matches.group(1)
                if is_valid_medical_range('ap_lo', val):
                    results['ap_lo'] = val
            elif key == "bp_combined":
                sys_val, dia_val = matches.group(1), matches.group(2)
                if 'ap_hi' not in results and is_valid_medical_range('ap_hi', sys_val):
                    results['ap_hi'] = sys_val
                if 'ap_lo' not in results and is_valid_medical_range('ap_lo', dia_val):
                    results['ap_lo'] = dia_val
            elif key == "gender":
                ...
                val = matches.group(1)
                if is_valid_medical_range(key, val):
                    results[key] = val
    return results
...
def preprocess_input(data):
    ...
    if chol < 200:
        data_copy['Cholesterol'] = 1
    elif 200 <= chol < 240:
        data_copy['Cholesterol'] = 2
    else:
        data_copy['Cholesterol'] = 3
    ...
    # 4. Build DataFrame
    input_data = {
        'Age ': [int(data_copy.get('age', 0))],
        'Gender': [data_copy['Gender']],
        'Height': [int(data_copy.get('height', 0))],
        'Weight': [float(data_copy.get('weight', 0))],
        'ap_hi': [int(data_copy.get('ap_hi', 0))],
        'ap_lo': [int(data_copy.get('ap_lo', 0))],
        'Cholesterol': [data_copy['Cholesterol']],
        'Gluc': [data_copy['Gluc']],
        'Smoke': [data_copy['Smoke']],
        'Alco': [data_copy['Alco']],
        'Active': [data_copy['Active']]
    }
...
def profile():
    history = (
        ...
        .order_by(Analysis.date_posted.desc())
        .limit(5)
        .all()
    )
...
def predict():
    ...
    for f in required_fields:
        if not data.get(f):
            return jsonify({'error': f'Missing {f}. Please fill all fields.'}), 400
    ...
    try:
        ...
        data['glucose'] = int(data['glucose'])

        # 2. Clinical Safety Net
        clinical_result = calculate_clinical_risk(data)

        if clinical_result:
            pred, prob = clinical_result
        else:
            if model is None:
                return jsonify({'error': 'Prediction model not available on server.'}), 500

            processed = preprocess_input(data)
            pred = int(model.predict(processed)[0])
            prob = float(round(model.predict_proba(processed)[0][1] * 100, 2))

        # 3. Save History (with duplicate protection)
        if current_user.is_authenticated:
            five_sec_ago = datetime.utcnow() - timedelta(seconds=5)

            recent = Analysis.query.filter(
                Analysis.user_id == current_user.id,
                Analysis.date_posted >= five_sec_ago,
                Analysis.age == data['age'],
                Analysis.gender == data.get('gender'),
                Analysis.height == data['height'],
                Analysis.weight == data['weight'],
                Analysis.ap_hi == data['ap_hi'],
                Analysis.ap_lo == data['ap_lo'],
                Analysis.cholesterol == data['cholesterol'],
                Analysis.glucose == data['glucose'],
                Analysis.smoke == data.get('smoke'),
                Analysis.alco == data.get('alco'),
                Analysis.active == data.get('active')
            ).first()

            if recent:
                # Same inputs within last 5 seconds → treat as duplicate
                return jsonify({
...
def handle_feedback():
    ...
    with open('feedback.txt', 'a') as f:
        f.write(
            f"Name: {data.get('name') or current_user.username}\n"
